...
def plot_policy_session_traces(
    df: pd.DataFrame,
    *,
    session_col: str,
    order_col: str,
    ...
    actual_peak_col: str | None = None,
    score_col: str | None = None,
) -> list[Path]:
    out_dir = ensure_dir(output_dir)
    work = df.sort_values([session_col, order_col]).copy()
    session_ids = work[session_col].astype(str).drop_duplicates().tolist()
    if max_sessions is not None:
        session_ids = session_ids[:max_sessions]

    written: list[Path] = []
    for session_id in session_ids:
        g = work[work[session_col].astype(str) == session_id].copy().sort_values(order_col).reset_index(drop=True)
        x = np.arange(1, len(g) + 1)
        y = pd.to_numeric(g[y_col], errors="coerce").to_numpy()

        fig, ax = plt.subplots(figsize=(8, 4.5))
        ax.plot(x, y, marker="o", linewidth=1.8, label=y_col)
        ax.axhline(0.0, linewidth=1)

        stop_rows = g[g[action_col] == "stop"]
        if not stop_rows.empty:
            stop_game = int(stop_rows.index[0] + 1)
            stop_y = float(pd.to_numeric(stop_rows[y_col], errors="coerce").iloc[0])
            ax.axvline(stop_game, linestyle="--", linewidth=1.5, label="predicted_stop")
            ax.scatter([stop_game], [stop_y], marker="x", s=80)

        if actual_peak_col and actual_peak_col in g.columns:
            peak_game = int(pd.to_numeric(g[actual_peak_col], errors="coerce").idxmax() + 1)
            peak_y = float(pd.to_numeric(g[actual_peak_col], errors="coerce").max())
            ax.scatter([peak_game], [peak_y], s=50, marker="o", label="actual_peak")

        ax.set_title(f"Policy trace: {session_id}")
        ax.set_xlabel("game_in_session")
        ax.set_ylabel(y_col)

        if score_col and score_col in g.columns:
            ax2 = ax.twinx()
            score = pd.to_numeric(g[score_col], errors="coerce").to_numpy()
            ax2.plot(x, score, linestyle=":", alpha=0.6, label=score_col)
...
def plot_threshold_sweep(
    ...
    *,
    ...
    ax.plot(
        pd.to_numeric(sweep_df[threshold_col], errors="coerce"),
        pd.to_numeric(sweep_df[metric_col], errors="coerce"),
        marker="o",
    )
    ax.set_xlabel(threshold_col)
    ax.set_ylabel(metric_col)
    ax.set_title(f"Threshold sweep: {metric_col}")
    ax.axhline(0.0, linewidth=1)
    out = Path(output_path)
    out.parent.mkdir(parents=True, exist_ok=True)
    fig.tight_layout()
